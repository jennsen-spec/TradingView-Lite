...
- **Mobile : seuls les panneaux Volume et RSI** — ATR et RS retirés (ils mangeaient la hauteur ; le panneau prix passe de ~470 à ~513 px). Le réglage **enregistré n'est pas modifié** (`activeAtr`/`activeRs` restent à `true` dans `tvlike:indicators`) : le desktop retrouve ses 5 panneaux. ATR et RS sont aussi masqués du catalogue mobile, sinon les ajouter serait une action sans effet visible.
