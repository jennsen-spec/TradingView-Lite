...
|---|------|:---:|:---:|:---:|:---:|---|---|
| [**97**](docs/backlog/97-epopee-sources-de-marche.md) | **Épopée : Sources de marché** | ⚙️ | ~34 | XL | | 🔍 | Registre d'instruments + routage par fournisseur : Yahoo devient un fournisseur parmi six (FRED, multpl, Banque du Canada, BCE, CoinGecko). Débloque CAC40, or, pétrole, devises, courbe des taux `US10Y-US02Y` et PE du S&P 500 — les 10 lignes des captures TradingView du 05/09. 6 enfants, non numérotés ; **après l'enfant 2, les 10 lignes marchent**. |
...
| 94 | **Fiche backtest d'un titre** | 💼 | ? | — | *À affiner.* Prendre un titre — réel ou synthétique — et afficher ses informations de backtest. **Recoupe [#82](docs/backlog/82-analyse-technique-backtest-synth.md) et [#83](docs/backlog/83-prtflio-backtest-overlay-tvlite.md)** (en LATER) : à cadrer ensemble avant de sprinter, sinon on construit trois fois la même chose. |
| 83 | **PrtFlio Backtest + Overlay TVLite** | 💼 | ? | — | [ticket](docs/backlog/83-prtflio-backtest-overlay-tvlite.md). *Placeholder — à recadrer.* L'outillage : backtester des stratégies de portefeuille (labo) + **overlay** du résultat dans TVLite. **Trio à fusionner avec #93 et #94** : composer le portefeuille (93) → le backtester (83) → en lire les résultats (94). À cadrer ensemble avant de sprinter ; #82 (l'*exploration*, en LATER) touche le même terrain. |
| 95 | **Écran de chargement animé** | 🎨 | 2 | XS | [ticket](docs/backlog/95-ecran-de-chargement.md). Remplacer l'écran blanc du démarrage (~1 s mesuré, 2-4 s en 4G) par l'animation du logomark — le fichier source est `loading-iphone.html` à la racine. À écrire **en dur dans `index.html`** pour peindre avant le bundle. 2 questions ouvertes (moment du retrait, fond bleu vs thème). |
| **⏭️ THEN** | | | | | |
...
- **🎯 Épopée [#70 — Responsive mobile / iPad](docs/backlog/70-epopee-responsive.md)** *(20 pts)* — ✅ **close le 02/09/2026**. TVLite s'utilise au doigt, installée sur l'écran d'accueil de l'iPhone ; desktop inchangé. Enfants : [#84](docs/backlog/84-coquille-mobile.md) Coquille · [#89](docs/backlog/89-onglet-rapport-mobile.md) Onglet Rapport · [#85](docs/backlog/85-watchlist-mobile.md) Watchlist · [#86](docs/backlog/86-graphique-mobile.md) Graphique v1 · [#87](docs/backlog/87-molettes-mobile.md) Molettes · [#88](docs/backlog/88-pwa.md) PWA.
- **Rapport & synchro (02/09)** : [#92](docs/backlog/92-notification-push-rapport.md) **Notification iOS à la publication** (Web Push : bannière app fermée + pastille d'icône, ouverture directe sur le Rapport ; worker `push` seul, sans cache) · [#90](docs/backlog/90-pastille-nouveau-rapport.md) Pastille « nouveau rapport » (méta de signal, extinction à la 1ʳᵉ consultation) · [#91](docs/backlog/91-fusion-dessins-suppressions.md) Les suppressions de dessins se propagent enfin entre appareils (`seen` des deux côtés de la fusion). **Purge du 02/09** : les 244 dessins ont été supprimés à la demande de Jean ; les 7 ensembles sont conservés.
- **Pré-rapport (05/09)** : [#96](docs/backlog/96-pre-rapport.md) **Pré-rapport à J−1** — la veille du dernier jour civil (reculée au vendredi le week-end), le rapport porte en tête « si le mois se terminait aujourd'hui » : interrupteur provisoire, ordres de la prochaine séance, candidats ; notifie comme une vraie publication et disparaît quand le rapport du mois sort. Le tableau « À faire » devient **unique** (vendre / acheter / conserver) avec une colonne **Résultat** et un pied décomposé — le rapport définitif en hérite.
